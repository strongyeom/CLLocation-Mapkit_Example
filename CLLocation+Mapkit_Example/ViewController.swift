...
class ViewController: UIViewController {
    ...
    func filterMovieAnnotation(type: MovieTheaterEnum) {
        ...
        // ⭐️ 고차 함수와 삼항연산자를 이용한 코드 개선
        let filterdResult = type.rawValue == "전체영화관" ? theaterList.mapAnnotations : theaterList.mapAnnotations.filter { $0.type.contains(type.rawValue) }
...
extension ViewController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // 성공적으로 update가 되면 배열에 쌓이는데 그중에 가장 마지막에 쌓이는 정보를 가져옴
        ...
        if let previousCoordinate = self.previousCoordinate {
            ...
            // 현재 위치
            let point2 = CLLocationCoordinate2DMake(locations.last!.coordinate.latitude, locations.last!.coordinate.longitude)
            points.append(point1)
            points.append(point2)
            // 이전 위치와 현재위치를 MKPolyline를 사용해서 이거주기
            let lineDraw = MKPolyline(coordinates: points, count: points.count)
            // 맵뷰 위에 오버레이 형식으로 띄어주기
            self.mapView.addOverlay(lineDraw)
        }
        
        // ⭐️ 순서 중요
        // 현재 위치가 계속 바뀌기 때문에 previousCoordinate을 위의 조건문에 계속적으로 업데이트 하면서
        // 새로운 라인을 그릴 수 있게 해줌
...
extension ViewController : MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        print("난 이런 스타일로 랜더링 할거야 ")
        guard let polyLine = overlay as? MKPolyline else {
            print("라인을 그리지 못했습니다.")
            return MKOverlayRenderer()
        }
        
        // 라인의 색상 두께 , 알파 값을 커스텀 할 수 있음
